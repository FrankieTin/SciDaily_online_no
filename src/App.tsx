...
function AppContent() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-base font-sans text-text-main selection:bg-sage/20 relative">
      ...
      {/* 顶部应用栏 */}
      <div className="md:hidden flex-none bg-sage border-b border-sage/70 shadow-[0_10px_22px_rgba(70,92,102,0.16)] z-30 relative transition-colors duration-300 pt-safe">
        <div className="relative flex h-[58px] items-center justify-center px-4 pb-2">
          {mainNav === 'research' && (
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="absolute left-4 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-[16px] border border-white/28 bg-white/14 text-white shadow-[0_8px_18px_rgba(66,84,94,0.14),inset_0_1px_0_rgba(255,255,255,0.16)] backdrop-blur-[8px] transition-colors hover:bg-white/20"
            >
              {mobileMenuOpen ? <X size={21} /> : <Menu size={21} />}
            </button>
          )}
          <h1 className="m-0 flex items-center gap-2 font-serif text-[19px] leading-none font-bold tracking-[0.01em] text-white drop-shadow-[0_1px_1px_rgba(0,0,0,0.06)]">
            科研Daily
          </h1>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        {mainNav === 'research' ? (
          <>
            ...
            {/* Main Content Area */}
            <main ref={mainRef} className="flex-1 overflow-y-auto w-full pb-24 md:pb-0 relative">
      ...
      {/* 小程序专属底部导航栏 */}
      <div className="md:hidden flex-none fixed bottom-0 left-0 right-0 h-[82px] bg-card/92 backdrop-blur-xl border-t border-line flex justify-around px-2 z-40 pb-safe shadow-[0_-10px_24px_rgba(53,72,82,0.08)]">
         <button 
           onClick={() => setMainNav('research')}
           className={cn("flex-1 flex flex-col items-center justify-start gap-1 pt-3.5 transition-colors", mainNav === 'research' ? 'text-sage font-bold' : 'text-text-muted hover:text-sage')}
         >
           <Home size={24} strokeWidth={mainNav === 'research' ? 2.6 : 2.2} />
           <span className="text-[12px] tracking-[0.08em] leading-none">记录</span>
         </button>
         <button 
           onClick={() => setMainNav('user')}
           className={cn("flex-1 flex flex-col items-center justify-start gap-1 pt-3.5 transition-colors", mainNav === 'user' ? 'text-sage font-bold' : 'text-text-muted hover:text-sage')}
         >
           <User size={24} strokeWidth={mainNav === 'user' ? 2.6 : 2.2} />
           <span className="text-[12px] tracking-[0.08em] leading-none">用户</span>
         </button>
      </div>
    </div>
  );
